...
std::pair<string, string> const get_build_dirs(string const & abs_binary)
{
	...
	while (true) {
		...
		lyx::FileInfo const file(binary, true);
...
string const get_locale_dir(string const & system_support_dir)
{
	...
	lyx::FileInfo fi(path);
	if (fi.isOK() && fi.isDir())
		return path;

	// 3. Fall back to the hard-coded LOCALEDIR.
	path = LOCALEDIR;
	lyx::FileInfo fi2(path);
...
string const abs_path_from_binary_name(string const & exe)
{
	...
	if (abs_binary.empty()) {
		...
		lyxerr << format(_("Unable to determine the path to the "
		       ...
		       << std::endl;
#else
		lyxerr << _("Unable to determine the path to the "
			    "LyX binary from the command line ")
		       << exe
		       << std::endl;
#endif
		bail_out();
		  ...
		  string const & command_line_system_support_dir)
{
	...
	while (true) {
		...
		lyx::FileInfo const file(binary_dir, true);
		if (!file.isOK() || !file.isLink())
			break;
	...
	// 5. In desparation, try the hard-coded LYX_DIR.
	path = LYX_DIR;
...
#if USE_BOOST_FORMAT
	using boost::format;
	lyxerr << format(_("Unable to determine the system directory "
			   "having searched\n"
			   "\t%1%\n"
	       ...
	       << std::endl;
#else
	lyxerr << _("Unable to determine the system directory "
		    "having searched\n")
	       << '\t' << searched_dirs_str << '\n'
	       << _("Try the '-sysdir' command line parameter or "
		    "set the environment variable LYX_DIR_13x to the "
		    "LyX system directory containing the file "
		    "`chkconfig.ltx'.")
	       << std::endl;
#endif
...
#if USE_BOOST_FORMAT
		using boost::format;
		lyxerr << format(_("Invalid %1% switch.\n"
				   "Directory %2% does not contain %3%."))
			         % command_line_switch % dir % file
		       << std::endl;
#else
		lyxerr << _("Invalid ") << command_line_switch
		       << _(" switch.\n")
		       << _("Directory ") << dir
		       << _(" does not contain ") << file
		       << std::endl;
#endif
	}
...
#if USE_BOOST_FORMAT
		using boost::format;
		lyxerr << format(_("Invalid %1% environment variable.\n"
				   "Directory %2% does not contain %3%."))
			         % env_var % dir % file
		       << std::endl;
#else
		lyxerr << _("Invalid ") << env_var

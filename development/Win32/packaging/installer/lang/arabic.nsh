!insertmacro LANGFILE_EXT "Arabic"

${LangFileString} TEXT_INSTALL_CURRENTUSER "(Installed for Current User)"

${LangFileString} TEXT_WELCOME "ÓÊגטÏד ÇהÈÑזÇוÌ ÎהÇה ÙוהךÉ ÊÑדךÈ ÈÑזÇוÌ הךד ÈךÓÑ טÓחטהÉ.$\r$\n\
					  $\r$\n\
					  $_CLICK"

${LangFileString} TEXT_CONFIGURE_MIKTEXFNDB "Updating MiKTeX filename database..."
${LangFileString} TEXT_CONFIGURE_LYX "Configuring LyX (MiKTeX may download missing packages, this can take some time) ..."
${LangFileString} TEXT_CONFIGURE_PYTHON "Compiling Python scripts..."

${LangFileString} TEXT_FINISH_DESKTOP "Create desktop shortcut"
${LangFileString} TEXT_FINISH_WEBSITE "Visit lyx.org for the latest news, support and tips"

${LangFileString} FileTypeTitle "וÓÊזÏ-הךד"

${LangFileString} SecInstJabRefTitle "JabRef"
${LangFileString} SecAllUsersTitle "ÊÑדךÈ הדה ÇהוÓÊÎÏוךז¿"
${LangFileString} SecFileAssocTitle "והבÇÊ וÑÊÈ×É"
${LangFileString} SecDesktopTitle "ÑוÒ Ó×Í ÇהודÊÈ"

${LangFileString} SecCoreDescription "והבÇÊ הךד."
${LangFileString} SecInstJabRefDescription "וÏךÑ טÍÑÑ ÇהÈהטÚÑÇבךÇ ÇהÎÇÕÉ ÈוהבÇÊ BibTeX."
${LangFileString} SecAllUsersDescription "ÊÑדךÈ ÈÑזÇוÌ הךד הדה ÇהוÓÊÎÏוךז Ãו ההוÓÊÎÏו ÇהÍÇהך בג×."
${LangFileString} SecFileAssocDescription "ÇהוהבÇÊ ÐÇÊ ÇהÇוÊÏÇÏ .lyx ÓךÊו בÊÍחÇ ÊהגÇÆךÇכ ÈÈÑזÇוÌ הךד."
${LangFileString} SecDesktopDescription "ÑוÒ הךד Ùהי Ó×Í ÇהודÊÈ."

${LangFileString} EnterLaTeXHeader1 'ÊטÒךÙÉ-הÊךד'
${LangFileString} EnterLaTeXHeader2 'ÇÌÙה ÊטÒךÙÉ-הÊךד ÇהÊטÒךÙÉ ÇהוÓÊÎÏו בך הךד.'
${LangFileString} EnterLaTeXFolder 'ךודזד חזÇ ÇÎÊךÇÑ ÇהוÓÇÑ ההוהב  $\"latex.exe$\" טÍÏÏ הÐהד  \
					   ÊטÒךÙÉ-הÊךד ÇהÐך ךÌÈ ÇÓÊÎÏÇוח בך הךד.$\r$\n\
					   ÇÐÇ דזÊ הÇ ÊÓÊÎÏו הÊךד בÇז הךד הÇ ךÓÊ×ךÙ ÇÎÑÇÌ ÇהוÓÊזÏÇÊ!$\r$\n\
					   $\r$\n\
					   Êו ÇהÙËטÑ Ùהי ÊטÒךÙÉ-הÊךד \
					   $\"$LaTeXName$\" בך זØÇוד טÊÌÏ וÓÇÑח בך ÇהÇÓבה.'
${LangFileString} EnterLaTeXFolderNone 'בךוÇ הך וÓÇÑ Çהוהב  $\"latex.exe$\". טÍÏÏ הÐהד \
					       ÊטÒךÙÉ-הÊךד ÇהÐך ךÌÈ ÇÓÊÎÏÇוח בך הךד.$\r$\n\
					       ÇÐÇ דזÊ הÇ ÊÓÊÎÏו הÊךד בÅז הךד הÇ ךÓÊ×ךÙ ÇÎÑÇÌ ÇהוÓÊזÏÇÊ!$\r$\n\
					       $\r$\n\
					       הו ךÊו ÇהÙËטÑ Ùהי ÊטÒךÙÉ-הÊךד בך זØÇוד.'
${LangFileString} PathName 'וÓÇÑ Çהוהב  $\"latex.exe$\"'
${LangFileString} DontUseLaTeX "הÇ ÊÓÊÎÏו הÊךד"
${LangFileString} InvalidLaTeXFolder 'Çהוהב $\"latex.exe$\" הךÓÊ בך ÇהוÓÇÑ ÇהוÍÏÏ.'

${LangFileString} LatexInfo 'ÓךÊו ÇהÈÏÁ ÈÊÑדךÈ ÊטÒךÙÉ-הÊךד ÈÈÑזÇוÌ $\"MiKTeX$\".$\r$\n\
			            ÇזגÑ Ùהי ÒÑ ÇהÊÇהך ההÅÓÊוÑÇÑ בך ÙוהךÉ ÇהÊÑדךÈ.$\r$\n\
				    $\r$\n\
				    !!! בÖהÇכ ÇÓÊÎÏו ÌוךÙ ÇהÎךÇÑÇÊ ÇהÇבÊÑÇÖךÉ ÙזÏ ÊÑדךÈ MiKTeX !!!'
${LangFileString} LatexError1 'הו ךÊו ÇהÙËטÑ Ùהי ÊטÒךÙÉ-הÊךד!$\r$\n\
                      		      הÇ ךודז ÇÓÊÎÏÇו הךד ÈÏטז ÊטÒךÙÉ הÊךד וËה MiKTeX!$\r$\n\
				      ÓךÊו ÇÌחÇÖ ÇהÊÑדךÈ.'

${LangFileString} HunspellFailed 'Download of dictionary for language $\"$R3$\" failed.'
${LangFileString} ThesaurusFailed 'Download of thesaurus for language $\"$R3$\" failed.'

${LangFileString} JabRefInfo 'ÓךÊו ÇהÂז ÊÑדךÈ ÈÑזÇוÌ JabRef.$\r$\n\
				     ÊÓÊ×ךÙ ÇÎÊךÇÑ דה ÇהÎךÇÑÇÊ ÇËזÇÁ ÊÑדךÈ ÈÑזÇוÌ JabRef.'
${LangFileString} JabRefError 'הו ךזÌÍ ÊÑדךÈ ÈÑזÇוÌ JabRef!$\r$\n\
		                      ÓÊÓÊוÑ ÙוהךÉ ÇהÊÑדךÈ.$\r$\n\
				      ÍÇטה ÊÑדךÈ JabRef הÇÍגÇכ.'

${LangFileString} LatexConfigInfo "ÇהÙוהךÉ ÇהÊÇהךÉ ÓÊÓÊÚÑג ÈÙÖ ÇהטגÊ."

${LangFileString} MiKTeXPathInfo "הדך ךדטז דה וÓÊÎÏו גÇÏÑÇ Ùהי ÊÎÕךÕ miktex הÇÍגÇכ ÍÓÈ ÇÍÊךÇÌÇÊח $\r$\n\
					 וז ÇהÖÑטÑך דÊÇÈÉ ÕהÇÍךÇÊ ÇהוÓÊÎÏוךז בך וÌהÏ ÊÑדךÈ MiKTeX$\r$\n\
					 $MiKTeXPath $\r$\n\
					 טבך ÇהוÌהÏÇÊ ÇהבÑÙךÉ."
${LangFileString} MiKTeXInfo 'ÊטÒךÙÉ-הÊךד MiKTeX ÊÓÊÎÏו ÇךÖÇכ וז גÈה הךד.$\r$\n\
				     טוז ÇהוטÕי Èח הÊËÈךÊ ÇהÊÍÏךËÇÊ ÇהוÊÇÍÉ הא miktex - ÅÓÊÎÏÇו ÈÑזÇוÌ $\"וÙÇהÌ ÊÍÏךË miktex$\"$\r$\n\
				     גÈה ÇÓÊÎÏÇו הךד הÃטה וÑÉ.$\r$\n\
				     חה ÊÑÚÈ ÈÇהÊÍגג וז ÇהÊÍÏךËÇÊ ÇהÂז¿'

${LangFileString} ModifyingConfigureFailed "Could not set 'path_prefix' in the configure script"
${LangFileString} RunConfigureFailed "הÇ ךודז ÊזבךÐ Çהא configure script"
${LangFileString} NotAdmin "ךÌÈ Ãז Êדטז ÈÕהÇÍךÉ ÇהוÏךÑ הדך ההגךÇו ÈÙוהךÉ ÇהÊÑדךÈ!"
${LangFileString} InstallRunning "Êו ÈÇהבÙה ÇהÈÏÁ ÈÙוהךÉ ÇהÊÑדךÈ!"
${LangFileString} StillInstalled "Êו ${APP_SERIES_KEY2} ÈÇהבÙה ÊÑדךÈ ÈÑזÇוÌ הךד הÐÇ Ùהךד ÇטהÇכ ÇהÚÇÁ ÊÑדךÈ ÇהזÓÎÉÇהÓÇÈגÉ."

${LangFileString} MultipleIndexesNotAvailable "The support for using several indexes in a document will not be available because$\r$\n\
					       MiKTeX was installed with administrator privileges but you are installing LyX without them."
${LangFileString} MetafileNotAvailable "The LyX support for images in the format EMF or WMF will not be$\r$\n\
					available because that requires to install a software printer to$\r$\n\
					Windows which is only possible with administrator privileges."

${LangFileString} FinishPageMessage "ÊחÇזךזÇ! Êו ÊÑדךÈ הךד ÈזÌÇÍ.$\r$\n\
					    $\r$\n\
					    (ÊÔÚךה הךד הÃטה וÑÉ גÏ ךÃÎÐ ÙÏÉ ËטÇזך.)"
${LangFileString} FinishPageRun "ÊÔÚךה הךד"

${LangFileString} UnNotInRegistryLabel "הו ךÊו ÇהÙËטÑ Ùהי הךד בך ÇהÑךÌÓÊÑך.$\r$\n\
					       הו ךÊו ÍÐב ÇÎÊÕÇÑÇÊ הךד וז Ó×Í ÇהודÊÈ טגÇÆוÉ ÇהÈÑÇוÌ."
${LangFileString} UnInstallRunning "ךÌÈ ÇÚהÇג הךד ÇטהÇכ!"
${LangFileString} UnNotAdminLabel "ךÌÈ Ãז Êוהד ÕהÇÍךÇÊ ÇהוÏךÑ ההÈÏÁ ÈÙוהךÉ ÇהÍÐב!"
${LangFileString} UnReallyRemoveLabel "חה ÊÑÚÈ ÈÍÐב ÈÑזÇוÌ הךד ÈÇהדÇוה וÙ דה ÇהדוÈטזזÊ ÇהÎÇÕ Èח¿"
${LangFileString} UnLyXPreferencesTitle 'LyX$\'s ÊבÖךהÇÊ ÇהוÓÊÎÏו'

${LangFileString} SecUnMiKTeXDescription "Êו ÇÒÇהÉ ÊטÒÙ- הÊךד MiKTeX."
${LangFileString} SecUnJabRefDescription "Êו ÇÒÇהÉ וÏךÑ ÇהÈÇÈהטÚÑÇבךÇ JabRef."
${LangFileString} SecUnPreferencesDescription 'ÍÐב וÌהÏ ÊÔדךה הךד$\r$\n\
					   $\"$AppPre\username\$AppSuff\${APP_DIR_USERDATA}$\"$\r$\n\
					   הדה ÇהוÓÊÎÏוךז.'
${LangFileString} SecUnProgramFilesDescription "ÇÒÇהÉ הךד טדה ÇהדוÈטזזÊ ÇהÎÇÕÉ Èח."

